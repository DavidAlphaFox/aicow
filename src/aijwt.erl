...
decode_jwt(#{header := Header, claims := Claims} = Context) ->
    ...
    try
        [HeaderJSON, ClaimsJSON] =
            Decoded = [jiffy_decode_safe(ai_base64:decode(X,Base64Options)) || X <- [Header, Claims]],
...
%% Decoding helpers
%%
-spec jiffy_decode_safe(binary()) -> map() | invalid.
%% @private
...
jwt_sign_with_crypto({hmac, Crypto}, Payload, Key) ->
    Base64Options = #{url => true},
    ai_base64:encode(crypto:hmac(Crypto, Key, Payload),Base64Options);
...
jwt_sign_with_crypto({rsa, Crypto}, Payload, Key) ->
    Base64Options = #{url => true},
    ai_base64:encode(public_key:sign(Payload, Crypto, Key),Base64Options);
